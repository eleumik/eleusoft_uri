...
/**
 * {@link URIProvider} that wraps a java.net.URI performing some corrections
 * 
...
public class Java4URIProvider implements URIProvider
{
    ...
    private URI __createURI2(String scheme,
        String auth,
        String path,
        String query,
        String fragment) throws URIException
    {
        try
        {
            return createURI(buildURIString(scheme,
                auth,
        ...
        String fragment) throws URIException
    {
        ...
        {
            throw new URIException("RFC3986 3.3 if an URI " + "does not contain an authority component then "
                + "the path cannot begin with [//]",
                path);
        }
        else
        {
            ...
            try
            {
                return new JDK14URI(new java.net.URI(scheme,
                    auth,
                    path,
                    query,
                    fragment));
    ...
    private static class JDK14URI extends AbstractJDKURI
    {
        ...
        JDK14URI(String escaped) throws java.net.URISyntaxException,
            URIException
        {
        ...
        protected URI _normalize() throws URIException
        {
            ...
            if (path == null)
            {
                ...
                || path.equals(".."))
            {
                // This is the case gone over the root, throw error or use an
                // empty path
                // java.net.URI handles in this way the paths over the root..
                if (isStrict())
                    throwExceptionForStrict("Cannot normalize a path over the root: [" + path
                        + "]");
                path = "";
                modified = true;
            ...
            if (modified)
            {
                ...
                {
                    /**
                     * return new JDK14URI( new
                     * java.net.URI(normjuri.getScheme(),
                     * normjuri.getAuthority(), path, query,
                     * normjuri.getFragment()));
                     **/
                    return new JDK14URI(buildURIString(normjuri.getScheme(),
                ...
                {
                    throw new IllegalStateException("Implementation error. Cannot apport modifications, path:[" + path
                        + "] query:["
                        + query
                        + "]");
                }
        ...
        protected URI _resolve(URI relative) throws URIException
        {
            ...
            if (isRelativePath())
                throw new URIException("This is a relative-path URI [ " + toString()
                    + "], cannot resolve with rel URI ["
                    + relative
                    + "]",
                    toString());
            ...
            try
            {
                ...
                if (relative.isPathEmpty() && relative.getAuthority() == null)
                {
                    ...
                    final String toUSEAuth = relURIAuth == null ? getAuthority()
                        : relURIAuth;
                    ...
                    // Return this absolute uri with authority, query and
                    // fragment
                    // as determined
                    // System.out.println("NO AUTH ON REL RESOLVED PATH :" +
                    // getPath());
                    // TODO : Return path "/" if this is absolute and has no
                    // path ??
                ...
                else
                {
                    ...
                    final java.net.URI resolved = toBeUsedBaseURI.resolve(new java.net.URI(relative.toString()));
                    ...
                    if (path.indexOf("/.") != -1)
                    {
                        ...
                        // if (path==null || || path.startsWith(".."))
                        {
                            return new JDK14URI(buildURIString(resolved.getScheme(),
                                ...
                                relativeURIFragment));
                            // throw new
                            // URIException("Path error, over the root:" + path,
                            // path);
                        }
            ...
            {
                throw new URIException("Cannot relativize:" + relative.toReadableURI(),
                    relative.toString(),
                    use);
            }
        }
        ...
        public final URI removeFragment() throws URIException
        {
            try
            {
                return !hasFragment() ? this
                    : new JDK14URI(new java.net.URI(buildURIString(uri.getScheme(),
                        uri.getAuthority(),
                        uri.getRawPath(),
                        uri.getRawQuery(),
                        null)));
            ...
            catch (java.net.URISyntaxException use)
            {
                ...
                throw new URIException("Cannot remove fragment:" + toReadableURI(),
                    toString(),
                    use);
            }
        }
